...
describe('Test EntityManager persist()', () => {
    ...
    it('can create a new entity', (done) => {
        const entityManager = serviceManager.get('entity.manager');
        const test = new TestEntity();
        test.set('unique_data', '12345');
        test.set('duplicated_data', '12345');

        (async () => {
            await entityManager.persist(test);
            await entityManager.flush();
            done();
        })();
    });

    it('can create multiple entity', (done) => {
        const entityManager = serviceManager.get('entity.manager');
        const test = new TestEntity();
        test.set('unique_data', '12345');
        test.set('duplicated_data', '12345');
        const test1 = new TestEntity();
        test1.set('unique_data', '123456');
        test1.set('duplicated_data', '12345');

        (async () => {
            await entityManager.persist(test);
            await entityManager.persist(test1);
            await entityManager.flush();
            assert(test.get('id') === 3, '新增失敗');
            assert(test1.get('id') === 4, '新增失敗');
            done();
        })();
    });

    it('can update an existed entity', (done) => {
        const entityManager = serviceManager.get('entity.manager');
        ...
        testEntityRepository.findOneBy({id: 1})
            .then(async (existedEntity) => {
                existedEntity.set('unique_data', '2222');
                await entityManager.persist(existedEntity);
                await entityManager.flush(existedEntity);
            })
            .then(() => testEntityRepository.findOneBy({id: 1}))
            .then((updatedEntity) => {
                assert(updatedEntity.get('unique_data') === '2222', '更新資料失敗');
                done();
            })
    });

    it('can update multiple entity', (done) => {
        const entityManager = serviceManager.get('entity.manager');
        const repositoryLoader = serviceManager.get('repository.loader');
        const testEntityRepository = repositoryLoader.basic('test_entity');
        ...
        testEntityRepository.findOneBy({id: 1})
            ...
            .then(async (existedEntity) => {
                existedEntity.set('unique_data', '2223');
                await entityManager.persist(existedEntity);
                await entityManager.flush(existedEntity);
            })
            .then(() => testEntityRepository.findOneBy({id: 2}))
            .then((updatedEntity) => {
                assert(updatedEntity.get('unique_data') === '2223', '更新資料失敗');
                done();
            })
    });

    it('can create and update entity', (done) => {
        const entityManager = serviceManager.get('entity.manager');
        const repositoryLoader = serviceManager.get('repository.loader');
        const testEntityRepository = repositoryLoader.basic('test_entity');
        ...
        testEntityRepository.findOneBy({id: 1})
            ...
            .then(() => testEntityRepository.findOneBy({id: 1}))
            .then((updatedEntity) => {
                assert(updatedEntity.get('unique_data') === '2222', '更改資料失敗');
            })
            .then(() => testEntityRepository.findOneBy({id: 3}))
            .then((createdEntity) => {
                assert(createdEntity.get('unique_data') === '12345', '新增失敗');
            })
            .then(() => {
                done();
            })
